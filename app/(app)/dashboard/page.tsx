...
// Wrap in Suspense at export level for static export compatibility
...
import { type JsonBlob, formatBytes, computeSize } from '@/lib/types';
...
function DashboardContent() {
  ...
  const stats = React.useMemo(() => {
    ...
    const shared = blobs.filter((b) => b.visibility === 'shared' || b.share_count > 0).length;
    const publicBlobs = blobs.filter((b) => b.visibility === 'public').length;
    const privateBlobs = blobs.filter((b) => b.visibility === 'private').length;
    const favorites = blobs.filter((b) => b.is_favorite).length;
    const archived = blobs.filter((b) => b.status === 'archived').length;
    const totalSize = blobs.reduce((sum, b) => sum + (b.size_bytes || 0), 0);
    return { total, shared, publicBlobs, privateBlobs, favorites, archived, totalSize };
  }, [blobs]);
  ...
  const handleToggleFavorite = async (blob: JsonBlob) => {
    try {
      const { error } = await supabase
        .from('blobs')
        .update({ is_favorite: !blob.is_favorite })
        .eq('id', blob.id);
      if (error) throw error;
      setBlobs((prev) =>
        prev.map((b) => (b.id === blob.id ? { ...b, is_favorite: !b.is_favorite } : b))
      );
    } catch (e) {
      toast({
        title: 'Update failed',
  ...
  const handleArchive = async (blob: JsonBlob) => {
    ...
    try {
      ...
      if (error) throw error;
      setBlobs((prev) =>
        prev.map((b) => (b.id === blob.id ? { ...b, status: newStatus } : b))
      );
      ...
      <header className="flex items-center justify-between gap-4 border-b border-border/50 px-6 py-3 bg-card/30">
        <div className="flex items-center gap-3 flex-1 max-w-md">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Search blobs, tags, content..."
              value={search}
...
function BlobCard({
  ...
  return (
    <Card className="glass-card p-4 hover:border-primary/30 hover:shadow-lg transition-all group">
      <div className="flex items-start justify-between gap-2">
        <Link href={`/editor/${blob.id}`} className="min-w-0 flex-1">
          ...
          {blob.description && (
            <p className="text-xs text-muted-foreground line-clamp-1 mb-2">{blob.description}</p>
          )}
        </Link>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem onClick={() => onToggleFavorite(blob)}>
              ...
              {blob.is_favorite ? 'Unfavorite' : 'Favorite'}
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => onDuplicate(blob)}>
              <Copy className="mr-2 h-4 w-4" />
              Duplicate
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => onDownload(blob)}>
      ...
      <div className="flex flex-wrap items-center gap-1.5 mt-3">
        ...
        {blob.is_favorite && (
          <Badge variant="outline" className="text-chart-5 border-chart-5/30">
            <Star className="mr-1 h-2.5 w-2.5 fill-chart-5" />
            Favorite
          </Badge>
        )}
        {blob.status === 'archived' && (
      ...
      <div className="flex items-center gap-3 mt-3 text-xs text-muted-foreground">
        <span className="flex items-center gap-1">
          ...
          {new Date(blob.updated_at).toLocaleDateString()}
        </span>
...
function BlobRow({
  ...
  return (
    <div className="flex items-center gap-3 glass-card rounded-lg p-3 hover:border-primary/30 transition-colors group">
      <button onClick={() => onToggleFavorite(blob)} className="shrink-0">
        <Star
          className={cn(
            'h-4 w-4 transition-colors',
            blob.is_favorite ? 'fill-chart-5 text-chart-5' : 'text-muted-foreground hover:text-chart-5'
          )}
        />
      </button>
      <Link href={`/editor/${blob.id}`} className="flex items-center gap-2 min-w-0 flex-1">
        <FileJson className="h-4 w-4 text-primary shrink-0" />
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-sm text-muted-foreground mt-1 max-w-sm">
        Create your first JSON blob to get started, or try adjusting your filters.
      </p>
